...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      if (error.message === 'This prestation already exists in the service') {
        return res.status(400).json(new ApiResponse(null, error.message));
      }
      next(error);
    }
  }
...
router.delete(
  ...
  async (req, res, next) => {
    try {
      const { service, prestation } = req.query;
      
      const result = await servprestService.deleteServicePrestation(service, prestation);
      
      if (!result) {
        return res.status(404).json(new ApiResponse(null, "Association not found"));
      }
      
      res.json(new ApiResponse(null, "Prestation removed from service"));
    } catch (error) {
      next(error);
    }
  }
...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const prestations = await servprestService.getPrestationsByService(serviceName);
      const totalElements = await ServicePrestation.countDocuments(filters);
